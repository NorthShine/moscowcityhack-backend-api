from src.db import users, whitelist, database
...
async def update_url_from_whitelist(url_id, new_url):
    query = whitelist.update(url=new_url).where(whitelist.id == url_id)
    await database.execute(query)


async def delete_url_from_whitelist(url_id):
    query = whitelist.delete().where(whitelist.id == url_id)
    await database.execute(query)
